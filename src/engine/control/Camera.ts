export class Camera {
    ...
    attachControls() {
        ...
        this.canvas.addEventListener('wheel', e => {
            ...
            // 1. Mouse to World relative to center
            const rect = this.canvas.getBoundingClientRect();
            const mx = e.clientX - rect.left;
            const my = e.clientY - rect.top;

            // Screen coords centered at 0,0
            const screenX = mx - this.canvas.width / 2;
            const screenY = my - this.canvas.height / 2;

            // Current Scale
            const viewW = this.worldWidth / this.zoom;
            const pxScale = viewW / this.canvas.width;

            // World offset of mouse from center
            const worldDx = screenX * pxScale;
            const worldDy = screenY * pxScale;
