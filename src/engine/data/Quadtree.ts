...
export class Quadtree {
    ...
    constructor(data: Uint8ClampedArray, width: number, height: number, threshold: number = 20) {
        this.data = data;
        this.width = width;
        this.height = height;
    ...
    queryRange(minX: number, minY: number, maxX: number, maxY: number): QuadNode[] {
        ...
        for (let i = 0; i < this.nodes.length; i++) {
            ...
            if (n.x < maxX && n.x + n.size > minX &&
                n.y < maxY && n.y + n.size > minY) {
                result.push(n);
            }
        }
        return result;
